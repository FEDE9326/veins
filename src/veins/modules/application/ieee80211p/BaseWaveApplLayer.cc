...
/*
 * CREATED new Type message ALERT
 */
...
WaveShortMessage*  BaseWaveApplLayer::prepareWSM(std::string name, int lengthBits, t_channel channel, int priority, int rcvId, int serial,std::string alert_id) {
	...
	// inserted this field for ack
	wsm->setAlertReceivedId(alert_id);
	...
	if (name =="alert"){
	    DBG << "Creating Alert with Priority " << priority << " at Applayer at " << wsm->getTimestamp() << std::endl;
	}
...
void BaseWaveApplLayer::handleLowerMsg(cMessage* msg) {
	...
	else if (std::string(wsm->getName()) == "alert"){
	    onAlert(wsm);
	}
	else {
...
void BaseWaveApplLayer::handleSelfMsg(cMessage* msg) {
	switch (msg->getKind()) {
		case SEND_BEACON_EVT: {
			sendWSM(prepareWSM("beacon", beaconLengthBits, type_CCH, beaconPriority, 0, -1,"progetto"));
