...
class BaseWaveApplLayer : public BaseApplLayer {
	...
	protected:
		...
		virtual WaveShortMessage* prepareWSM(std::string name, int dataLengthBits, t_channel channel, int priority, int rcvId, int serial=0, std::string alert_id="");
		virtual void sendWSM(WaveShortMessage* wsm);
		virtual void onBeacon(WaveShortMessage* wsm) = 0;
		virtual void onData(WaveShortMessage* wsm) = 0;
        virtual void onAlert(WaveShortMessage* wsm) = 0;
