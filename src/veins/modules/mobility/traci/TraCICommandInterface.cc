...
void TraCICommandInterface::Vehicle::updateDirection(Coord pos,simtime_t timestamp,int id){
    this->storage.updateStorage(VehicleData(pos,timestamp,id));
}

bool TraCICommandInterface::Vehicle::verifyPresence(long id){

    return this->storage.verifyPresence(id);
